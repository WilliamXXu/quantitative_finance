...
class TestStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            ind2=[self.flag,self.mfi[0] < 30,self.pl(price=self.priceStart)<-3,len(self)-self.barStart<14,self.position.size<10]

            ind=[self.flag,self.mfi[0] < 30,self.pl(price=self.priceStart)<-6,len(self)-self.barStart<21,self.position.size<10]
            
            #print(self.broker.get_cash())
            #print(self.position.size)
            if np.product(ind) or np.product(ind2):
                self.log('BUY CREATE, %.2f' % self.dataclose[0])
                #print('buy')
                #print('already')
                #print(self.position.size)
