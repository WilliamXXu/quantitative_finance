...
def get_src_home_url(asset_type: str):
    return f'https://www.nasdaq.com/market-activity/{asset_type}'

def get_options_price_url(tickersymbol:str, expiration_date:str,option_type, asset_type: str):
    option_type = enum_OptionType_to_string(option_type)
    return f"https://api.nasdaq.com/api/quote/{tickersymbol}/option-chain?assetclass={asset_type}&limit=1000&fromdate={expiration_date}&todate={expiration_date}&excode=oprac&callput={option_type}&money=all&type=all"

def get_options_contract_url(tickersymbol:str, recordID:str, asset_type: str):
    return f"https://api.nasdaq.com/api/quote/{tickersymbol}/option-chain?assetclass={asset_type}&recordID={recordID}"
